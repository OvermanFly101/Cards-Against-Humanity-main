...
# Renders the Muhammad page (INCOMPLETE).
@app.route('/muhammad_page')
def muhammad_page():
    pass
...
def register():
    if not User.valid_register(request.form):
        return redirect('/')
    data = {
        'character_name': request.form['character_name'], # Change to whatever Muhammad put for users table (INCOMPLETE).
...
# Renders the dashboard page when you open this in the default browser.
